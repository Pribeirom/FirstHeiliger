...
public:
	...
	bool init(const char* windowTitle = "Default Game Window", int x = 50, int y = 50, int w = 640, int h = 480, int initFlag = SDL_INIT_EVERYTHING, int windowFlag = SDL_WINDOW_SHOWN, int rendererFlag = SDL_RENDERER_ACCELERATED);
	void clean();
	
	//image management (loading, drawing, etc.)
	void clearCanvas(){ SDL_RenderClear(m_pRenderer); };
	void presentCanvas(){ SDL_RenderPresent(m_pRenderer); };
